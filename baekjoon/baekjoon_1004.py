...
def get_distance(PointA, PointB):
    return sqrt(pow(PointB[1] - PointA[1], 2) + pow(PointB[0] - PointA[0], 2))
...
for _ in range(int(sys.stdin.readline().rstrip())):
    ...
    result = 0
    for _ in range(int(sys.stdin.readline().rstrip())):
        planetary_systemX, planetary_systemY, radius = map(int, sys.stdin.readline().rstrip().split())
        planetary_system = (planetary_systemX, planetary_systemY)
        distance_start = get_distance(start, planetary_system)
        distance_end = get_distance(end, planetary_system)
        if (distance_start > radius > distance_end) or (distance_start < radius < distance_end):
            result += 1
    print(result)
