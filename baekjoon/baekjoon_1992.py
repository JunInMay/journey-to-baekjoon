...
image = [list(map(int, list(sys.stdin.readline().rstrip()))) for _ in range(int(sys.stdin.readline().rstrip()))]

def check_image(image):
    checker = abs(image[0][0] - 1)
    for line in image:
        if checker in line:
            return -1
    return image[0][0]
...
def slice_image(image):
    ...
    if check != -1:
        ...
        for level in range(2):
            img1, img2 = [[], []]
            for i in range(l//2):
                img1.append(image[level * (l // 2) + i][:l // 2])
                img2.append(image[level * (l // 2) + i][l // 2:])
            result += slice_image(img1)
            result += slice_image(img2)
    result += ")"
    return result

print(slice_image(image))