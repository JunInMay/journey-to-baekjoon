...
cups = [int(sys.stdin.readline().rstrip()) for _ in range(int(sys.stdin.readline().rstrip()))]
memo = [[0, cups[0], cups[0]]]
# 안마시는 경우의 최대값, 연속으로 마시는 경우, 한칸 띄워서 마시는 경우
if len(cups) >= 2:
    memo.append([max(memo[0]), memo[0][2]+cups[1], cups[1]])
for i in range(2, len(cups)):
    memo.append([max(memo[i-1]), memo[i-1][2]+cups[i], max(memo[i-2])+cups[i]])
